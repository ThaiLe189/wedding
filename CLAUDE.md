...
- `src/components/Layout.jsx` holds chrome shared across the page: `SiteIntro` (the opening invitation screen), `Header`, `Footer`.
- `src/intro/wedding-intro.js` is a framework-free Web Component (`<wedding-intro>`, shadow DOM, own CSS) that renders the 2.5D folding invitation card. `SiteIntro` creates it imperatively (to pass the `assets` object before mount), sets the `auto-enter` attribute, and listens for `invitation-enter`; the component fires that right after the card finishes opening (the inside of the card is intentionally empty — no pop-up layer or text), and `App.jsx` then fades the overlay (`.site-intro.is-leaving`) and unmounts it. Its images live in `src/design/assets/intro/` (webp). Keep its markup/CSS inside the component — the only site-side styles are the overlay shell rules in `animations.css`.
